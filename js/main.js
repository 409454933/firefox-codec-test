...
virtualBackground.addEventListener('click', streamBackgroundTest);
...
remoteVideo.addEventListener('resize', () => {
	console.log(
		`Remote video size changed to ${remoteVideo.videoWidth}x${remoteVideo.videoHeight} - Time since pageload ${performance.now().toFixed(0)}ms`
		);
...
async function onCreateOfferSuccess(desc) {
	...
	desc.sdp = setSessionLevelMediaLine(desc.sdp)
	console.log(`Offer from pc1\n${setSessionLevelMediaLine(desc.sdp)}`);
    ...
    if (parsedSdp.media && parsedSdp.media.length) {
        for (let i = 0; i < parsedSdp.media.length; i++) {
            ...
            if (media.type === 'audio') {
                ...
                //     if (This.attr.roCodec) {
                //         // cons.info('remote codec ' + This.attr.roCodec)
                //         SDPTools.removeCodecByName(parsedSdp, i, [This.attr.roCodec], true)
                //         This.attr.roCodec = null
                //     }
                //     if (This.attr.setCodec) {
                //         log.info('set codec ' + This.attr.setCodec)
                //         SDPTools.removeCodecByName(parsedSdp, i, [This.attr.setCodec], true)
                //         This.attr.setCodec = null
                //     }
                // }
            }
        }
...
async function streamBackgroundTest(){
	...
	let virtualBackgroundStream = await navigator.mediaDevices.getUserMedia(constraints)
